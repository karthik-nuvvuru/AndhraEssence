...
from tests.conftest import auth_header, get_customer_address, get_restaurant_and_items
...
class TestCreateOrder:
    ...
    async def test_create_order_success(self, client, customer_token, seeded_db):
        ...
        restaurant_id, menu_item_id = await get_restaurant_and_items(
    ...
    async def test_create_order_invalid_restaurant(
...
class TestListOrders:
    ...
    async def test_list_orders_success(self, client, customer_token, seeded_db):
        ...
        response = await client.get(
            "/api/v1/orders", headers=auth_header(customer_token)
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_list_orders_filter_by_status(
        ...
        response = await client.get(
            "/api/v1/orders?status=pending", headers=auth_header(customer_token)
        )
        assert response.status_code == 200
    ...
    async def test_list_orders_pagination(self, client, customer_token, seeded_db):
        ...
        response = await client.get(
            "/api/v1/orders?page=1&limit=5", headers=auth_header(customer_token)
        )
        assert response.status_code == 200
        data = response.json()
        assert data["page"] == 1
...
class TestGetOrder:
    ...
    async def test_get_order_access_denied(
        ...
        response = await client.get(
            f"/api/v1/orders/{order_id}", headers=auth_header(owner_token)
        )
...
class TestUpdateOrderStatus:
    ...
    async def test_update_order_status_confirm(
        self, client, owner_token, customer_token, seeded_db
    ):
